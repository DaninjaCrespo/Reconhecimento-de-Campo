    ...
    inimigos.forEach((ini) => {
        ...
        if (ini.tipo === 'obstaculo') {
            ...
            corpoHtml = `
                ...
                ${ini.tipo === 'screamer' ? `<div class="controle-grupo"><label>Tam. Rastro: <span class="valor-slider" id="val-rastro-${ini.id}">${ini.maxRastro} pt</span></label><input type="range" id="rastro-${ini.id}" min="100" max="2500" step="100" value="${ini.maxRastro}" oninput="attCard(${ini.id})"></div>` : ''}
